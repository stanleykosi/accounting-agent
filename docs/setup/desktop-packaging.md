...
<!--
Purpose: Document the canonical packaging flow for the Tauri desktop shell and bundled Next.js sidecar.
Scope: Host prerequisites, staged resource layout, first-run setup behavior, and operator recovery guidance for desktop installers.
...
The packaged shell then launches:

- the bundled Node runtime from `resources/runtime/`
- the bundled Next.js server from `resources/desktop-ui/server.js`
- the first-run setup route at `http://127.0.0.1:3210/setup`
- a normalized runtime environment assembled from the current process environment and, when present, a `desktop-shell.env` file in the app config directory
...
If `ACCOUNTING_AGENT_API_URL` is omitted, the shell derives it from `api_host`, `api_port`, and `runtime_api_base_path`.
...
The Tauri shell opens a lightweight local launcher page first, then waits for the bundled standalone server to respond on loopback. Once the server is ready, the shell navigates into the setup experience.
...
- If the packaged app opens but stays on the setup screen, start the local demo stack with the shared operator scripts and use the in-app refresh button.
