...
def test_configure_observability_passes_optional_otlp_headers(monkeypatch) -> None:
    """Ensure Grafana-style endpoints use OTLP/HTTP and carry hosted OTLP headers."""
    ...
    def fake_http_metric_exporter(**kwargs: object) -> object:
        captured_arguments["http_metric_exporter"] = dict(kwargs)
        return {"kind": "metric_exporter", "kwargs": kwargs}

    monkeypatch.setattr(otel_module, "_CONFIGURED_PID", None)
    monkeypatch.setattr(otel_module, "HttpOTLPSpanExporter", fake_http_span_exporter)
    monkeypatch.setattr(otel_module, "HttpOTLPMetricExporter", fake_http_metric_exporter)
    monkeypatch.setattr(
        otel_module,
        "GrpcOTLPSpanExporter",
        lambda **kwargs: (_ for _ in ()).throw(
            AssertionError("gRPC span exporter should not be created for Grafana OTLP endpoints")
        ),
    )
    monkeypatch.setattr(
        otel_module,
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            AssertionError("gRPC metric exporter should not be created for Grafana OTLP endpoints")
        ),
    )
    ...
    assert captured_arguments["http_span_exporter"] == {
    ...
    assert captured_arguments["http_metric_exporter"] == {
        "endpoint": "https://otlp-gateway.example.com/otlp/v1/metrics",
        "headers": {
            "authorization": "Basic abc123",
            "x-scope-orgid": "tenant-42",
        },
...
def test_configure_observability_uses_grpc_for_collector_style_endpoint(monkeypatch) -> None:
    """Ensure collector-style endpoints without a path keep using OTLP/gRPC."""
    ...
    def fake_grpc_span_exporter(**kwargs: object) -> object:
        ...
        return {"kind": "grpc_span_exporter", "kwargs": kwargs}

    def fake_grpc_metric_exporter(**kwargs: object) -> object:
        captured_arguments["grpc_metric_exporter"] = dict(kwargs)
        return {"kind": "grpc_metric_exporter", "kwargs": kwargs}

    monkeypatch.setattr(otel_module, "_CONFIGURED_PID", None)
    monkeypatch.setattr(otel_module, "GrpcOTLPSpanExporter", fake_grpc_span_exporter)
    monkeypatch.setattr(otel_module, "GrpcOTLPMetricExporter", fake_grpc_metric_exporter)
    monkeypatch.setattr(
        otel_module,
        "HttpOTLPSpanExporter",
        lambda **kwargs: (_ for _ in ()).throw(
            AssertionError("HTTP span exporter should not be created for collector-style endpoints")
        ),
    )
    monkeypatch.setattr(
        otel_module,
    ...
    assert captured_arguments["grpc_span_exporter"] == {
        "endpoint": "http://collector.internal:4317",
        "headers": None,
        "insecure": True,
    }
    assert captured_arguments["grpc_metric_exporter"] == {
        "endpoint": "http://collector.internal:4317",
        "headers": None,
        "insecure": True,
    }
...
def test_configure_observability_skips_otlp_exporters_without_endpoint(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "HttpOTLPSpanExporter",
        lambda **kwargs: (_ for _ in ()).throw(
            AssertionError("HTTP span exporter should not be created")
        ),
    )
    monkeypatch.setattr(
        otel_module,
        "HttpOTLPMetricExporter",
        lambda **kwargs: (_ for _ in ()).throw(
            AssertionError("HTTP metric exporter should not be created")
        ),
    )
    monkeypatch.setattr(otel_module.trace, "set_tracer_provider", lambda provider: None)
    monkeypatch.setattr(otel_module.metrics, "set_meter_provider", lambda provider: None)
